...
static void find_corners(float x,
    ...
    switch (rotation) {
		case 90:
           *y1 = 1.0 - fmax(0.0, x - (w / 2));
           *x1 = fmax(0.0, y - (h / 2));
           *y2 = 1.0 - fmin(1.0, x + (w / 2));
           *x2 = fmin(1.0, y + (h / 2));
           break;
        case 180:
        ...
        case 270:
           *y1 = fmax(0.0, x - (w / 2));
           *x1 = 1.0 - fmax(0.0, y - (h / 2));
           *y2 = fmin(1.0, x + (w / 2));
           *x2 = 1.0 - fmin(1.0, y + (h / 2));
           break;
        default:
            *x1 = fmax(0.0, x - (w / 2));
            *y1 = fmax(0.0, y - (h / 2));
...
int main(int argc, char** argv) {
	...
	//int rotation = ax_parameter_get_int(axparameter_handle, "root.ImageSource.I0.Rotation");
	//GError *error = NULL;
	//gchar *value = NULL;
	int rotation = 90;/*
	if (ax_parameter_get(axparameter_handle, "root.ImageSource.I0.Rotation", &value, &error)) 
	{
		rotation = atoi(value);
		printf("Rotation = %d\n", rotation);
	}*/
	...
	unsigned int actual_stream_width  = stream_width;
	unsigned int actual_stream_height = stream_height;
	//int rotation = (int)get_stream_rotation(image_provider);
	if (rotation == 90 || rotation == 270) 
	{
		actual_stream_width  = stream_height;
		actual_stream_height = stream_width;
		image_provider =
			create_img_provider(actual_stream_width, actual_stream_height, 2, VDO_FORMAT_YUV);
		model_provider = create_model_provider(model_params->input_width,
															model_params->input_height,
															actual_stream_width,
															actual_stream_height,
